import { FormEvent, ReactElement, createRef, useState, useEffect } from "react";
...
export default function NewConversationView({ autoInitAddress }: { autoInitAddress?: string }): ReactElement {
  ...
  useEffect(() => {
    if (autoInitAddress) {
      initConversation(autoInitAddress);
    }
  }, [autoInitAddress]);

  async function initConversation(address: string) {
    try {
